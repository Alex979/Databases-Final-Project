...
if(isset($_POST['submit'])){
	...
	if($completeForm == true){
		...
		$zip = $_POST["zip"];
		$complete = true;
//		$query ="INSERT INTO applicant(uid, ssn, uname, fname, lname, street, city, state, email, phone, zip, complete) VALUES ('$uid', '$ssn', '$uname', '$fName', '$lName', '$street', '$city', '$state', '$email', '$phone', '$zip', '$complete')";
		$query = "UPDATE applicant SET ssn='$ssn', street='$street', city='$city', state='$state', phone='$phone', zip='$zip', complete='1' WHERE uid='$uid'" ;
